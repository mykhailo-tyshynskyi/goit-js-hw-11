...
`}).join("");n.innerHTML=s,g.refresh()}function h(){n.innerHTML=""}function L(){document.querySelector(".loader").classList.remove("isHiden")}function w(){document.querySelector(".loader").classList.add("isHiden")}const S=document.querySelector(".form"),b=document.querySelector('[name="search-text"]');S.addEventListener("submit",q);function q(a){a.preventDefault();const s=b.value.trim();s!==""&&(h(),L(),p(s).then(r=>{if(r.length===0)throw new Error;y(r),a.target.reset()}).catch(r=>{f.error({title:"Error",message:"Sorry, there are no images matching your search query. Please try again!",position:"center"})}).finally(()=>{w()}))}
