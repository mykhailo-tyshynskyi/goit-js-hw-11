...
`}).join("");c.innerHTML=s,y.refresh()}function L(){c.innerHTML=""}function w(){document.querySelector(".loader").classList.remove("isHiden")}function n(){document.querySelector(".loader").classList.add("isHiden")}const S=document.querySelector(".form"),b=document.querySelector('[name="search-text"]');S.addEventListener("submit",q);function q(a){a.preventDefault();const s=b.value.trim();if(s===""){n();return}else L(),w(),g(s).then(r=>{if(r.length===0)throw new Error;n(),h(r),a.target.reset()}).catch(r=>{n(),p.error({title:"Error",message:"Sorry, there are no images matching your search query. Please try again!",position:"center"})})}
